...
架構設計見 [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md)，後續功能規劃見 [`docs/ROADMAP.md`](docs/ROADMAP.md)。
...
├── components/{ui,OrderTab,StatusControl,PeopleList,Summary}.jsx
...
零阻力是這個工具的價值來源。改用三種憑證：`join_code`（分享用短碼）、`edit_token`（改自己的單）、`admin_token`（發起人關團、代刪、批次改狀態）。後兩者存在 localStorage，換裝置會遺失，此時由發起人代為處理。權限判斷一律在後端。
